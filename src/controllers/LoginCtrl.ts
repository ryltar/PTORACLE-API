...
export class LoginCtrl {
    ...
    async connect(@BodyParams() body:any): Promise<Array<IPersonne[]>|any> {
        ...
        let personnes = await getExecution("SELECT * from personne WHERE login = :identifiant AND mdp = :motdepasse ", [body.login, body.mdp])
        ...
        tab.push(personne)
        // checking role
        if (await getExecution("SELECT * from administrateur WHERE idadministrateur = :identifiant ", [personne.idPersonne])) //if the person is an admin
            var role = "admin"
        else if (await getExecution("SELECT * from client WHERE idadministrateur = :identifiant ", [personne.idPersonne])) { //if the person is a client
            var role = "client"
